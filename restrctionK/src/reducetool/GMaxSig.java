 ...
 * @author 		xinyan
 */
public class GMaxSig {
	//data(listC,listD,listU,String[][])
	public static ArrayList<Integer> maxInSigD(EqualClassPara data,ArrayList<ArrayList<Integer>> equaD){
		...
		list1.addAll(GD.DCD(data2,equaD));
		...
		for(Integer item:data.R){
			...
			list2.clear();
			list2.addAll(GD.DCD(data2,equaD));
	...
	//data(red, ,listU,String[][])
	public static ArrayList<Integer> maxOutSigD(EqualClassPara data,
			ArrayList<Integer> C,
		...
		for(Integer item:c_red){
			...
			data1.setListR(reda);	
			list2.clear();
			list2.addAll(GD.DCD(data1,equaD));
	...
	public static ArrayList<Integer> maxInSig(EqualClassPara data){
		...
		listR_a.addAll(data.R);
		//存放red，red_a生成等价类
		ArrayList<Integer> list1=GD.DX(data,Config.degree);
		ArrayList<Integer> list2=new ArrayList<Integer>();
		double max=-Double.MAX_VALUE,maxTemp=0;
		Integer temp=0;
		...
		for(Integer item:data.R){
			listR_a.remove(item);
			data1.setListR(listR_a);
			list2=GD.DX(data1,Config.degree);
