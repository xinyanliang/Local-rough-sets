	...
	public static void larc(){
	    ...
	    while(stopNew<stop){
	    	...
	    	equalClassXiUired=ListTool.equalityClassU(
	    			NearR.nearR(new EqualClassPara(red,X,Ui, matrix[0])
	    			));
	    	//5.4
	    	red=LMaxSig.maxOutSig(new EqualClassPara(red,X,equalClassXiUired,matrix[0]),listR);
	    	//5.5
	    	Pi.add(red);
	    	stopNew =LD.DX(new EqualClassPara(red,X,equalClassXiUired,matrix[0]), Config.degree).size();
	    	//System.out.println(stopNew+" "+stop);
	    }
